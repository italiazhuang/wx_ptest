...
namespace IZ.WebFileManager
{
    ...
    public sealed class FileManager : FileManagerControlBase, IPostBackEventHandler
    {
      ...
      [Bindable(true)]
        ...
        public bool HideEXT
        {
            get { return FileView.HideEXT; }
            set { FileView.HideEXT = value; }
        }
            ...
            if (eventArgument == "Upload")
            {
                ...
                for (var i = 0; i < Page.Request.Files.Count; i++)
                {
                    if (Page.Request.Files.GetKey(i) == ClientID + "_Upload")
                    {
                        ...
                        string NA = postedFile.FileName;

                        string name = NA.Substring(NA.LastIndexOf("."), NA.Length - NA.LastIndexOf(".")).ToLower();

                        if (name == ".png" || name == ".jpg" || name == ".jpge" || name == ".mp3" || name == ".amr" || name == ".bmp" || name == ".gif" || name == ".wav")
                        {
